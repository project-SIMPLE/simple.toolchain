...
public class WizardPageProperties extends WizardPage {
	...
	public void createControl(final Composite parent) {
		...
		btnEdit.addListener(SWT.Selection, e -> {
			if (e.type == SWT.Selection && properiesList.getSelectionIndex() >= 0) {
				...
				for (String id : booleanAreas.keySet()) {
					booleanAreas.get(id)
							.setSelection(currentValues.get(id) != null && "true".equals(currentValues.get(id)));
		...
		btnAddupdate.addListener(SWT.Selection, e -> {
			if (e.type == SWT.Selection) {
				...
				if (!name.isBlank()) {
					...
					definedProperties.put(name, new HashMap<>(currentValues));
					initValue();
					updateList();
				}
		...
		btnReset.addListener(SWT.Selection, e -> { if (e.type == SWT.Selection) { initValue(); } });
	...
	void initValue() {
		...
		for (String id : textAreas.keySet()) { currentValues.put(id, textAreas.get(id).getText()); }
		for (String id : booleanAreas.keySet()) {
	...
	public WizardPageSpeciesToSend getWpStS() { return wpStS; }

	public void setWpStS(final WizardPageSpeciesToSend wpStS) { this.wpStS = wpStS; }

	public void setPlayerPage(final WizardPagePlayer playerPage) {

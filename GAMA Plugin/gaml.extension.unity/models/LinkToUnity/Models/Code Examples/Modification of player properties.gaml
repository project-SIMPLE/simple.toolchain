/**
* Name: ModificationOfPlayerProperties
* Show how to modifiy the Unity player properties from GAMA It works with the Scene "Assets/Scenes/Code Example/Modify Player Properties" from the Unity Template
* Author: Patrick Taillandier
* Tags: Unity, player movement, camera
*/


model ModificationOfPlayerProperties
...
species unity_linker parent: abstract_unity_linker {
	...
	list<point> init_locations <- [world.location + {0,0,100.0}];
...
species unity_player parent: abstract_unity_player {
	...
	float x_movement_speed <- 3.0; 
	
	float y_movement_speed <- 3.0;
	
	bool x_movement_strafe <- false;
	
	float camera_clipping_planes_far <- 100000.0;
	float camera_clipping_planes_near <- 1.0;
	
	float movement_min_y <- 10.0;

	float movement_max_y <- 5000.0;
